...
allAnnotations.eachWithIndex{ anno, x ->
    try {
        ...
        // 创建自定义的Overlay
        def customOverlay = new HierarchyOverlay(viewer.getImageRegionStore(), viewer.getOverlayOptions(), imageData) {
            @Override
            protected boolean shouldShowObject(PathObject pathObject) {
                // 只显示当前注释
                return pathObject == anno
            }
        }
